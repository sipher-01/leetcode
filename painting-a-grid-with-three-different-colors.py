class Solution:
    ...
    def solve(self,prev_idx,rem_column, m):
        if rem_column==0:
            return 1
        ...
        for i in range(len(self.colour_string)):
            ...
            for j in range(m):
                if cur_column[j]==prev_column[j]:
                    ...
                    break
            if valid:
                ways = (ways + self.solve(i,rem_column-1,m))%self.mod
        
        self.memo[rem_column][prev_idx] = ways
        return ways

    def colorTheGrid(self, m: int, n: int) -> int:
        #step 1 : generate all column strings
        self.generateColumn("",'#',0,m)
        ...
        for i in range(k):
            ...
            res = (res + self.solve(i,n-1,m))%self.mod
