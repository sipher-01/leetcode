...
public:
    int longestBalanced(vector<int>& nums) {
        ...
        for(int i = 0; i < n; i++){
            ...
            if(mp.count(nums[i])){
                int prev = mp[nums[i]];
                st.range_update(0, 0, prev, 0, n-1, -val);  // FIXED: query=[0,prev], segment=[0,n-1]
            }
            
            st.range_update(0, 0, i, 0, n-1, val);  // FIXED: query=[0,i], segment=[0,n-1]
